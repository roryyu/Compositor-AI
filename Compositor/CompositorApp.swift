    ...
    var body: some Scene {
        Window("Compositor", id: "editor") {
            ...
            .commands {
                ...
                CommandMenu("AI") {
                    Button("AI Settings…") {
                        AIPanels.shared.showSettings()
                    }
                    Button("Generate Image…") {
                        AIPanels.shared.showGenerate(session: session)
                    }
                        .configuredKeyboardShortcut("g", modifiers: [.command, .shift])
                    Button("Analyze Canvas…") {
                        AIPanels.shared.showAnalyze(session: session)
                    }
                    Divider()
                    Button("Edit with AI…") {
                        AIPanels.shared.showAgent(session: session)
                    }
                }
            }
    }
